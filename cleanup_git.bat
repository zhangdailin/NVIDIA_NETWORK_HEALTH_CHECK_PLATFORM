@echo off
REM Git 仓库清理脚本 (Windows)

echo 开始清理 Git 仓库...
echo.

REM 1. 从 Git 历史中移除 node_modules
...
REM 3. 从 Git 历史中移除 backend/.venv
echo 3. 移除 backend/.venv 历史...
git filter-branch --force --index-filter "git rm -r --cached --ignore-unmatch backend/.venv" --prune-empty --tag-name-filter cat -- --all

REM 4. 清理 reflog 和垃圾回收
echo 4. 清理 reflog...
